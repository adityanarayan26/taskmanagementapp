...
import { CirclePlus, Loader, CircleCheck, Trash2, Pencil } from 'lucide-react';
const App = () => {
    ...
    <div className='min-h-screen w-full bg-gradient-to-tl from-indigo-800 to-indigo-900 flex flex-col items-center'>
      ...
      <div className='h-fit w-full flex flex-wrap  justify-start gap-5 pl-32 mt-20'>
        ...
        {Alltask && Alltask.map((item, id) => (
          <div className='h-fit border bg-zinc-400 p-5  rounded-xl flex flex-col justify-center items-start' key={id}>
            ...
            <div className='flex gap-2'>
              ...
              <button className='bg-zinc-600 p-1 rounded-lg cursor-pointer mt-4'><Pencil className='text-emerald-200' onClick={()=>handleEdit(item)} /></button>
            </div>
          </div>
        ))}
      </div>
